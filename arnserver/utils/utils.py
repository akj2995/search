...
def _load_doc_mat_desc_new(src_sim_doc_array, src_sim_topic_array, qids, qid_cwid_label, doc_mat_dir, qid_topic_idf,
                           qid_desc_idf, usetopic, usedesc, maxqlen, h5fn=None):
    assert usetopic or usedesc, "must use at least one of topic or desc"
...
def sample_train_data_weighted(qid_wlen_cwid_mat, qid_cwid_label, \
    ...
    print("random_seed:", random_seed, ",n_query_terms:", n_query_terms, ", NUM_NEG:", NUM_NEG, ",n_dims:", n_dims)
    ...
    while 1:
        ...
        for label in label_counter:
            ...
            if cur == 0:
                print("	selected_qids:", selected_qids)
...
def load_test_data(qids, rawdoc_mat_dir, qid_cwid_label, N_GRAMS, param_val):
    ...
    logger.info('Test data: {0} {1} {2}'.format([(wlen, getmat(doc_vec[wlen]).shape) for wlen in doc_vec], q_idfs.shape,
                                                len(cwids)))
...
def load_test_data_new(qids, rawdoc_mat_dir, qid_cwid_label, N_GRAMS, param_val, src_sim_doc_array, src_sim_topic_array,
    ...
    qid_cwid_rmat, qid_term_idf = _load_doc_mat_desc_new(src_sim_doc_array, src_sim_topic_array, qids, qid_cwid_label,
                                                         rawdoc_mat_dir, qid_topic_idf, \
                                                         qid_desc_idf, usetopic=param_val['ut'],
                                                         usedesc=param_val['ud'],
                                                         maxqlen=MAX_QUERY_LENGTH)

    # print("qid_cwid_rmat : ", qid_topic_idf)
    # print("qid_term_idf : ", qid_term_idf)
    qid_cwid_rqexpmat = None
    qid_cwid_mat, qid_ext_idfarr, qid_context = convert_cwid_udim_simmat(qids, qid_cwid_rmat, select_pos_func,
                                                                         qid_term_idf, qid_cwid_rqexpmat,
